...
build_elm_reactor:
	cd elm_reactor && npm install && npm run build
...
build_elm-live:
	cd elm-live && npm install && npm run build
...
.PHONY: build_parcel
build_parcel:
	cd parcel && npm install && npx parcel build index.html --no-source-maps --public-url /elm-app-templates/parcel/
...
.PHONY: build_esbuild
build_esbuild:
	cd esbuild && npm install && npm run build
...
.PHONY: build_vite
build_vite:
	cd vite && npm install && npx vite build --base /elm-app-templates/vite/
...
build_snowpack:
	cd snowpack && npm install && npx snowpack build --baseUrl /elm-app-templates/snowpack/
